...
const ListAutores = () => {
  ...
  const confirmarEliminar = async () => {
    ...
    try {
      await axios.delete(`http://localhost:5000/api/authors/${modalEliminar.autor._id}`);
      setAutores(prevAutores => prevAutores.filter(autor => autor._id !== modalEliminar.autor._id));
      cerrarModalEliminar();
  ...
  const confirmarEditar = async () => {
    ...
    try {
      const dataToSend = {
        ...formEditData,
        obras: formEditData.obras ? formEditData.obras.split(',').map(obra => obra.trim()) : []
      };

      const response = await axios.put(`http://localhost:5000/api/authors/${modalEditar.autor._id}`, dataToSend);
      
      // Actualizar la lista local
      setAutores(prevAutores =>
        prevAutores.map(autor =>
          autor._id === modalEditar.autor._id ? response.data.author : autor
        )
      );
  ...
  return (
    <div className="container listautores-container">
      <div className="listautores-header">
        ...
        <button onClick={handleRegistrarAutor} className="btn btn-success">
